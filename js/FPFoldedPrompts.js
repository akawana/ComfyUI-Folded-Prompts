...
(function () {
    const AREAS = ["ALL", "AR1", "AR2", "AR3", "AR4", "AR5", "ALL_TAG"];
    ...
    function parseFolderTextToTree(rawText) {
        ...
        for (const rawLine of rawLines) {
            ...
            let area = "ALL";
            const singleAreaMatch = text.match(/^<(AR[1-5]|ALL)>\s*([\s\S]*?)\s*<\/>\s*$/);
            if (singleAreaMatch) {
                area = singleAreaMatch[1] === "ALL" ? "ALL_TAG" : singleAreaMatch[1];
            ...
            const openAreaMatch = text.match(/^<(AR[1-5]|ALL)>\s*(.*)$/);
            if (openAreaMatch) {
                areaBlockArea = openAreaMatch[1] === "ALL" ? "ALL_TAG" : openAreaMatch[1];
        ...
        function walk(itemsList, path) {
            for (const item of itemsList) {
                if (item.type === "folder") {
                    ...
                    // If the line has area (AR1..AR5 or ALL_TAG) — wrap in tag
                    if (item.area && item.area !== "ALL") {
                        const tagName = item.area === "ALL_TAG" ? "ALL" : item.area;
                        lineText = `<${tagName}>` + lineText + `</>`;
                    }
